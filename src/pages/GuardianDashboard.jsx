...
function getActiveReward(payload) {
  ...
  if (Array.isArray(payload)) {
    return (
      payload.find(
        (reward) => reward.active ?? reward.is_active ?? !reward.delivered,
      ) ||
      payload[0] ||
      null
    );
...
function getTaskPoints(log) {
  return (
    log.points ?? log.task?.points ?? log.task_points ?? log.taskPoints ?? 0
  );
}
...
function GuardianDashboard() {
          ...
          children.map(async (child) => {
            ...
            try {
              ...
              return {
                ...
                rewardPercent:
                  rewardTarget > 0
                    ? Math.min((rewardProgress / rewardTarget) * 100, 100)
                    : 0,
              };
            } catch {
  ...
  async function handleCreateChild(event) {
    ...
    if (!Number.isInteger(grade) || grade < 1 || grade > 12) {
      setChildFormError("Grade should be a whole number from 1 to 12.");
      return;
    }

    if (!Number.isInteger(age) || age < 5 || age > 18) {
      setChildFormError("Age should be a whole number from 5 to 18.");
      return;
    }
    ...
    try {
      ...
      setChildFormError(
        error.response?.data?.detail ||
          "We couldn't create that child profile yet.",
      );
    } finally {
        ...
        <header className="rounded-[30px] bg-[#2D6A4F] p-5 text-white shadow-[0_24px_65px_-30px_rgba(45,106,79,0.8)]">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex flex-col gap-2">
              <BackToKidsButton onClick={() => navigate("/")} variant="dark" />
              <button
                type="button"
                onClick={logout}
                ...
                <div className="mt-5 space-y-4">
                  <section>
                    ...
                    <p className="mt-2 text-xs text-[#1B1B1B]/50">
                      {child.totalCount} {getMustDoLabel(child.totalCount)}{" "}
                      tracked today
                    </p>
                  </section>
